...
for(j in 1:ncol(rmse_diff)){
    ...
    p_single[[j]] <- ggplot(rmse_diff_pooled, aes(x=RMSE_diff)) + geom_histogram() + labs(x = "Difference in error between mr.mash and enet", title=colnames(rmse_diff)[j]) + theme_cowplot()
    sumstats<- cbind(sumstats, summary(rmse_diff[, j]))
}
