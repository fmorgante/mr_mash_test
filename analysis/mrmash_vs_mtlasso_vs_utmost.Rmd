...
In all the analyses so far, we have used [Abhishek's implementation of the sparse multi-task LASSO](https://users.rcc.uchicago.edu/~aksarkar/gtex-pred/lasso.html) (called mtlasso here), the method that was used in [UTMOST](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6788740/). This is because the [original implementation](https://github.com/yiminghu/CTIMP) requires the data in an awkward format. Recently, the Ritchie Lab reimplemented the [method](https://github.com/RitchieLab/multi_tissue_twas_sim) using the original code from the UTMOST autors so that it takes is data in the standard format (i.e., Y in matrix form). The caveat is that this reimplementation does not allow for missing values in Y. In this website, we want to compare *mr.mash* to both the Ritchie Lab's implementation and Abhishek's implementation of UTMOST. Because the original UTMOST method only provides the estimated coefficients and does not provide an estimated intercept, we are going to compare the accuracy of the estimated coefficients for the three methods in simulations with independent predictors.  

We simulated five scenarios with n=500, p=1000, r=10, PVE=0.2, p_causal=randomly sampled between 1 and 10, independent residuals, and the following structure of  effects across conditions:
...
Accuracy of the estimated coefficients is measured as the mean squared error (MSE) to the true coefficients.

#### Equal effects across conditions

```{r equal effects simulations, fig.height=12, fig.width=15}
...
dsc_plots_time <- dsc_plots[which(dsc_plots$response==1),
...
```{r independent effects simulations, fig.height=12, fig.width=15}
dsc_out <- dscquery("output/mvreg_mrmash_vs_mtlasso_vs_utmost",
                    c("simulate.n", "simulate.p", "simulate.p_causal", "simulate.r",
                      "simulate.pve", "simulate.B_cor", "simulate.B_scale",
                      "simulate.X_cor", "simulate.X_scale",
...
```{r singleton effects simulations, fig.height=12, fig.width=15}
dsc_out <- dscquery("output/mvreg_mrmash_vs_mtlasso_vs_utmost",
                    c("simulate.n", "simulate.p", "simulate.p_causal", "simulate.r",
                      "simulate.pve", "simulate.B_cor", "simulate.B_scale",
                      "simulate.X_cor", "simulate.X_scale",
...
p <- ggplot(dsc_plots, aes_string(x = "response_fac", y = "score_value", fill = "method_fac")) +
        geom_boxplot(color = "black", outlier.size = 1, width = 0.85) +
        scale_fill_manual(values = colors) +
        labs(x = "Condition", y = "MSE", fill="Method") +
        theme_cowplot(font_size = 20)
...
```{r one block effects simulations, fig.height=12, fig.width=15}
dsc_out <- dscquery("output/mvreg_mrmash_vs_mtlasso_vs_utmost",
                    c("simulate.n", "simulate.p", "simulate.p_causal", "simulate.r",
                      "simulate.pve", "simulate.B_cor", "simulate.B_scale",
                      "simulate.X_cor", "simulate.X_scale",
...
```{r two blocks effects simulations, fig.height=12, fig.width=15}
dsc_out <- dscquery("output/mvreg_mrmash_vs_mtlasso_vs_utmost",
                    c("simulate.n", "simulate.p", "simulate.p_causal", "simulate.r",
                      "simulate.pve", "simulate.B_cor", "simulate.B_scale",
                      "simulate.X_cor", "simulate.X_scale",
