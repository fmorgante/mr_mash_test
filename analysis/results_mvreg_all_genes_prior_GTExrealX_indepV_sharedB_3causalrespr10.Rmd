---
title: "Simulations with GTEx genotypes and 3 causal responses with shared effects"
...
convert_dsc_to_dataframe_gtex <- function(dsc){
  ...
  n_genes <- n_elem/n_methods
  
  ###Compute replicates
  reptot <- rep(rep(1:n_genes, each=dsc$simulate.r[1]), n_methods)
...
dsc_out <- dscquery("output/mvreg_all_genes_prior_GTExrealX_indepV_sharedB_3causalrespr10", 
                    c("simulate.p_causal", "simulate.r", "simulate.pve", "simulate", "fit", "score", "score.err", "fit.time"), 
...
In summary, all the methods perform pretty similarly. Any flavor of *mr.mash* is on the lower side of the error. Using data-driven matrices or canonical is equivalent in this situation.

# PVE=0.05

```{r load data low pve, warning=FALSE, message=FALSE}
###Load the dsc results
dsc_out <- dscquery("output/mvreg_all_genes_prior_GTExrealX_indepV_sharedB_3causalrespr10_lowPVE", 
                    c("simulate.p_causal", "simulate.r", "simulate.pve", "simulate", "fit", "score", "score.err", "fit.time"), 
                    groups="fit: mr_mash_em_can, mr_mash_em_data, mr_mash_em_dataAndcan, mr_mash_em_dataAndcan_dropcomp, mlasso, mtlasso, enet",
                    verbose=FALSE)
...
In summary, all the methods perform even more similarly. Any flavor of *mr.mash* is on the lower side of the error. Using data-driven matrices seems to be a little worse than using canonical matrices in this situation.
